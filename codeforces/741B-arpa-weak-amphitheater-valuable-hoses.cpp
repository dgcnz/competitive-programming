// dp(i, cap) : maximum value with first i representatives and capacity cap
//
// dp(i, cap) = max (
//              dp(i - 1, cap),
//              dp(i - 1, cap - sum({wj | j <- V(i)})) + sum({vj | j <- V(i)})
//              {dp(i - 1, cap - wj) + vj | j <- V(i)}
//                  )
...
int solve(void)
{
    vector<vi> dp(n + 1, vi(c + 1, 0)); // define 0
    ...
    for (int u = 1; u <= n; ++u)
    {
        ...
        for (int cap = 0; cap <= c; ++cap)
        {
            ...
            ans = max(ans, dp[cur_rep - 1][cap]); // taking none

            if (cap - tw >= 0)
                ans = max(ans, dp[cur_rep - 1][cap - tw] + tb); // taking all
